...
for num in range(len(x_1)):
    pp1_fit.append(param_1[0]*x_1[num]+param_1[1])
array_pp1_fit = np.array(pp1_fit)

pp2_fit=[]
for num in range(len(x_2)):
    pp2_fit.append(param_2[0]*x_2[num]+param_2[1])
array_pp2_fit = np.array(pp2_fit)

pp3_fit=[]
for num in range(len(x_3)):
    pp3_fit.append(param_3[0]*x_3[num]+param_3[1])
...
ax.plot(x_1, array_pp1_fit, '-', markersize=100, c='red', label = rf'$({round(param_1[0], 3)})x{round(param_1[1], 3)}$')
ax.plot(x_2, array_pp2_fit, '-', markersize=100, c='blue', label = rf'$({round(param_2[0],     3)})x+{round(param_2[1], 3)}$')
ax.plot(x_3, array_pp3_fit, '-', markersize=100, c='green', label = rf'$({round(param_3[0],     3)})x+{round(param_3[1], 3)}$')
#ax.bar(x, y, width=0.025, edgecolor="#000000")
plt.xlabel(r"$\varepsilon$", fontsize = 18)
plt.ylabel(r"$ln[P( \Delta x) / P(- \Delta x)]$", fontsize = 18)
ax.legend()
fig.savefig("F2_fitcurve_strainhist.png")
plt.show()
