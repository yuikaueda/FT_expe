...
ax.plot(x_1, array_pp1_fit, '-', markersize=100, c='red', label = rf'$({round(param_1[0], 20)})x/(KT){round(param_1[1], 3)}$')
ax.plot(x_2, array_pp2_fit, '-', markersize=100, c='blue', label = rf'$({round(param_2[0],     20)})x/(KT)+{round(param_2[1], 3)}$')
ax.plot(x_3, array_pp3_fit, '-', markersize=100, c='green', label = rf'$({round(param_3[0],     20)})x/(KT)+{round(param_3[1], 3)}$')
#ax.bar(x, y, width=0.025, edgecolor="#000000")
plt.xlabel(r"$\varepsilon$", fontsize = 18)
plt.ylabel(r"$ln[P( \Delta x) / P(- \Delta x)]$", fontsize = 18)
ax.legend()
fig.savefig("ktF2_fitcurve_strainhist.png")
plt.show()
